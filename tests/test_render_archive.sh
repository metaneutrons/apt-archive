...
o2="$work/t1out2"
run --manifest "$m" --project demo --pool-dir "$p" --output-dir "$o2" --metadata-epoch $EPOCH >/dev/null
...
o4="$work/t4out"
run --manifest "$m4" --project demo --pool-dir "$p4" --output-dir "$o4" --metadata-epoch $EPOCH >/dev/null
for a in amd64 arm64; do
...
o5="$work/t5out"
run --manifest "$m5" --project demo --pool-dir "$p5" --output-dir "$o5" --metadata-epoch $EPOCH >/dev/null
