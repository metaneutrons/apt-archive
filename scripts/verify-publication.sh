#!/usr/bin/env bash
# Checks a published archive against its public URL.
#
#   verify-publication.sh --manifest domains/<host>/manifest.toml \
#                         --project <name> --archive-dir <local root> \
#                         --publication-epoch <epoch> \
#                         [--base-url <override>]
#
# What gets checked is what a client actually receives, not what was uploaded.
# No credentials, read-only.
#
#   1  fetch the keyring
#   2  fetch InRelease and check it with gpgv against that keyring
#   3  compare InRelease byte for byte against the local copy
#   4  per architecture, fetch Packages.gz and check it against the sum in
#      InRelease
#   5  fetch the same index under its by-hash path and compare
#   6  resolve the first Filename entry, fetch the package and compare it byte
#      for byte against the local file
#
# --base-url serves the tests, which serve the tree locally.
set -euo pipefail
...
# Fetch with a limit: a hostile origin must not be able to fill the checker.
get() {  # $1 = path relative to the base, $2 = target, $3 = size limit in bytes
...
step "    exactly one subkey, as intended"

step "2  InRelease and its signature"
...
# gpgv 2.5 verifies a clear signature but does not write out its plaintext
# portably with --output. Hence extract it separately with the same isolated
# keyring; the trust decision above stays with gpgv.
...
step "    gpgv accepts, signed by ${signer}, epoch ${signature_epoch}"

step "3  Byte-for-byte comparison against the local tree"
...
step "4  Indexes against the sums from InRelease"
...
PY
  ...
  step "    ${arch}: Packages.gz matches"

  # Exactly the path apt actually requests: the strongest hash from Release.
  # Measured on 1 September 2026, see apt-archive.md.
...
PY
  ...
  step "    ${arch}: by-hash/SHA512 identical"

  # Every Filename entry, not just the first. awk deliberately reads to EOF: an
  # early `exit` closes the pipe, lets gzip die with SIGPIPE 141 on large
  # indexes and, under pipefail, ends the whole follow-up check without our own
  # error message.
...
step "5  Every published package, byte for byte"
# A package listed in several architecture indexes, for instance with
# Architecture: all, is fetched only once.
